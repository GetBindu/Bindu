...
class InMemoryStorage(Storage[dict[str, Any]]):
    ...
    async def submit_task(self, context_id: UUID, message: Message) -> Task:
        ...
        if existing_task:
            ...
            existing_task["status"] = TaskStatus(state="submitted", timestamp=datetime.now(UTC).isoformat())

            return existing_task

        # Task doesn't exist - create new task
        task_status = TaskStatus(state="submitted", timestamp=datetime.now(UTC).isoformat())
        task = Task(
            id=task_id,
