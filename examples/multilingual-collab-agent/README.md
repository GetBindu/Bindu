...
- use Mem0 for optional persistent memory across sessions

The agent is implemented in [`main.py`](./main.py) and configured through [`agent_config.json`](./agent_config.json).
...
- Optional persistent memory with Mem0
...
- `MEM0_API_KEY` is optional and enables persistent memory features
- `MODEL_NAME` is optional and defaults to `openai/gpt-4o-mini`

## Setup

From `examples/multilingual-collab-agent`, run:
...
Based on [`agent_config.json`](./agent_config.json), the example listens on:
...
Defined in [`skills/research/skill.yaml`](./skills/research/skill.yaml).
...
Defined in [`skills/translate/skill.yaml`](./skills/translate/skill.yaml).
...
Defined in [`skills/collaborate/skill.yaml`](./skills/collaborate/skill.yaml).
...
At startup, [`main.py`](./main.py):

- loads environment variables from `.env`
- reads [`agent_config.json`](./agent_config.json)
- builds an Agno `Agent`
- attaches DuckDuckGo search tools
- attaches Mem0 tools when `MEM0_API_KEY` is available
...
- `OPENROUTER_API_KEY`
- `MEM0_API_KEY` if you want persistent memory enabled

### Mem0 unavailable

If Mem0 is not configured or initialization fails, the agent continues without persistent memory. This fallback is handled in [`main.py`](./main.py).
...
- [`main.py`](./main.py) for model logic, instructions, and tool wiring
- [`agent_config.json`](./agent_config.json) for deployment and runtime configuration
